...
<span class="line"><span style="color:#24292E;">         return self.get_size() == 0</span></span></code></pre></div><p>首先，通过 if 判断语句与栈顶的数据进行配对，也就是假设 self.data 假设它的最后的字符 + c 是不是配对，我们可以用这个方法判断在几个可选的条件里，它们的 组合是否符合完全配对的条件，如果符合这个时候就需要弹栈，也就是 self.pop，如果不符合就通过 self.push 进行压栈。</p><br><p>然后我们 return 一个 self.get_size 是否等于 0 来判断栈是否为空。这样我们就完成了第一个方法，当然这段代码肯定有各种各样的 bug，我们先运行一遍，发现问题解决问题，直到测试用例符合我们的预期。</p>`,33),i=n(`<p>我们先看第一步报错，显示 list index out of range，为什么会出现这个报错呢？是因为我们的数据一开始是空的，当 index 为 -1 肯定不行，数据为空抛出异常，这个时候我们就需要添加一个条件判断，如果栈一开始没有数据，我们不能直接使用这个方法，这时我们可以添加一个方法并进行封装。</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki github-dark vp-code-dark"><code><span class="line"><span style="color:#E1E4E8;">def top(self):</span></span>
...
<span class="line"><span style="color:#24292E;">return self</span></span></code></pre></div><p>接下来，通过 pre 在当前的节点进行插入，插入一个节点和以前的逻辑一样，首先创建一个节点，然后把当前节点的 next 指针等于 5 这个节点，然后定义一个新的节点 node.new 等于 LinkNode 并传入 data，最后让 node_next.nex 等于现在的 item，并让上一个节点 pre.next 等于 node_new，通过这样的方法节点就被正确的插入链表，然后运行 case。</p>`,17),f=n(`<p>我们可以看到得到了预期的结果。</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki github-dark vp-code-dark"><code><span class="line"><span style="color:#E1E4E8;">self.link.append(7).append(6)</span></span>
<span class="line"><span style="color:#E1E4E8;">link_data =[item.data for item in self.link.travel()]</span></span>
...
<span class="line"><span style="color:#24292E;">link_data =[item.data for item in self.link.travel()]</span></span>
<span class="line"><span style="color:#24292E;">assert link_data != [1,2,3,4,5,6,7]</span></span></code></pre></div><p>通过这个方法我们就可以发现使用order就可以很好的帮我们去维护这个有序的链表数据了。</p>`,3);function _(g,u,h,k,v,b){const s=l("Image");return p(),t("div",null,[r,a(s,{alt:"",src:"https://s0.lgstatic.com/i/image3/M01/64/73/CgpOIF470r-ASjlzAAL2-HzWbjI277.png"}),i,a(s,{alt:"",src:"https://s0.lgstatic.com/i/image3/M01/64/74/Cgq2xl470r-ANICIAAOQgrpr9l8446.png"}),d,a(s,{alt:"",src:"https://s0.lgstatic.com/i/image3/M01/64/73/CgpOIF470r-ACRiDAAHGQCT5II4290.png"}),E,a(s,{alt:"",src:"https://s0.lgstatic.com/i/image3/M01/64/73/CgpOIF470r-ANMQEAALzFUdNppc735.png"}),y,a(s,{alt:"",src:"https://s0.lgstatic.com/i/image3/M01/64/74/Cgq2xl470r-AbZj6AAGq631NcHM427.png"}),f])}const C=e(c,[["render",_]]);export{x as __pageData,C as default};
