...
<span class="line"><span style="color:#24292E;">        sef.travel(child)</span></span></code></pre></div><br><p>首先，第一步是递归改造，令传入的 node 等于 None，然后如果 node is None，这时就让 node 等于 self，这样就完成了第一步改造，第二步改造是增加默认参数。再次运行，测试用例没有问题。</p><br>`,24),i=l(`<br><p>但是这样打印效果仍然不是很好，并不能很好的看出层级关系，而且后面的断言也需要层级关系，所以我们接下来增加层级关系的处理逻辑。</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki github-dark vp-code-dark"><code><span class="line"><span style="color:#E1E4E8;">def travel(self.node=None,depth=1):</span></span>
<span class="line"><span style="color:#E1E4E8;">    #第一步递归改造</span></span>
<span class="line"><span style="color:#E1E4E8;">    #第二步增加默认参数改造</span></span>
...
<span class="line"><span style="color:#24292E;">    print(f&quot;{&#39;  &#39;*depth}{node} depth={depth}&quot;)</span></span></code></pre></div><br><p>然后我们使用 for node，depth in root.travel(): 进行打印，这样改造之后我们再次运行测试用例。</p><br>`,9),d=l(`<br><p>你可以看到现在只是生成了一个节点，为什么会只生成一个节点呢？是因为 yield 调用 travel 时候里面有个递归逻辑。</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki github-dark vp-code-dark"><code><span class="line"><span style="color:#E1E4E8;">for child in node.children:</span></span>
...
<span class="line"><span style="color:#24292E;">depth -=1</span></span></code></pre></div><br><p>但是已有的方法在递归中没有跑起来，所以我们需要添加一个 Python 的特殊字符 yield from，我们再次打印。</p><br>`,6),y=l(`<br><p>我们再次运行，一个基本的树结构就已经完全的打印出来了，有了这个树形结构我们自然要检查这个树形结构的层级对不对。</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki github-dark vp-code-dark"><code><span class="line"><span style="color:#E1E4E8;">assert tree_dict[&quot;x&quot;]==4</span></span>
...
<span class="line"><span style="color:#24292E;">    return</span></span></code></pre></div><br><p>当根节点出现时我们需要将其进行压栈，通过 stack.push 把当前的节点压入栈，根节点处理完之后，就到了 head 节点，这时就会进入 else 逻辑，它需要在原有 stack 中进行追加，因为 head 是 html 的子节点，所以这里我们使用 stack.top，top 可以获取栈顶的元素，获取到栈顶元素后，我们可以使用 children.append 方法对树的子节点进行添加，经过这样一步方法编写，head 就可以放到 html 里面了，以此类推。</p><br><p>什么时候需要弹栈呢，就是当 key 遇到一个 / 的时候，这时候 if 逻辑下的 pass 就变成了 stack.pop。</p><br><p>整个结构都处理完之后我们需要返回 root 节点，然后跑下测试用例验证逻辑是否正确。</p><br>`,26),h=s("br",null,null,-1),u=s("p",null,"这时报错，我们就需要去查找原因，我们可以看到测试用例需要一个 content，因为 create_from_string 是一个类方法，我已经传入了一个参数但是它提示还缺一个参数，原因在于 tree 结构有两个参数，第一个参数要传入一个类，从目前来看调用的方法里面，它使用的是 Tree 这个数据，也就是说你调用的参数的类型是不对的，那怎么办呢？我们对于使用类而不是实例直接进行调用时，需要将它改造成一个静态方法。",-1),_=s("br",null,null,-1),g=s("br",null,null,-1),f=s("p",null,"我们使用 classmethod 对 create_tree_string 进行注解，然后再使用一个 cls，我们将它改造成一个类方法。",-1),q=s("br",null,null,-1),b=s("br",null,null,-1),v=s("p",null,"处理完成之后我们运行 case，整个 case 完全可以通过，然后我们把打印逻辑的代码复制过来并打印下这个结构。",-1),k=s("br",null,null,-1),m=s("br",null,null,-1),T=s("p",null,"可以发现数据结构完全正确，到这里根据 html 数据生成多叉树的处理逻辑就全部讲完了。",-1),A=s("br",null,null,-1);function C(D,N,I,x,S,B){const n=e("Image");return o(),t("div",null,[r,a(n,{alt:"",src:"https://s0.lgstatic.com/i/image3/M01/66/81/CgpOIF5GdIKAIsZ1AADAOtimqRs873.png"}),i,a(n,{alt:"",src:"https://s0.lgstatic.com/i/image3/M01/66/81/Cgq2xl5GdIOAK2iIAADUdQhHwIQ158.png"}),E,a(n,{alt:"",src:"https://s0.lgstatic.com/i/image3/M01/66/81/CgpOIF5GdIOAQQJnAADIUkZj7wg648.png"}),d,a(n,{alt:"",src:"https://s0.lgstatic.com/i/image3/M01/66/81/Cgq2xl5GdIOASaR1AADIGqiGv1M869.png"}),y,a(n,{alt:"",src:"https://s0.lgstatic.com/i/image3/M01/66/81/CgpOIF5GdIOAFA6ZAAFGmXvabCo953.png"}),h,u,_,a(n,{alt:"",src:"https://s0.lgstatic.com/i/image3/M01/66/81/Cgq2xl5GdIOASL9dAADXKF9RsZk190.png"}),g,f,q,a(n,{alt:"",src:"https://s0.lgstatic.com/i/image3/M01/66/81/CgpOIF5GdIOAM3FBAAEkGsUFxqY064.png"}),b,v,k,a(n,{alt:"",src:"https://s0.lgstatic.com/i/image3/M01/66/81/Cgq2xl5GdIOAdN7TAAEINwwXC50534.png"}),m,T,A])}const P=p(c,[["render",C]]);export{O as __pageData,P as default};
