...
<span class="line"><span style="color:#24292E;">        return Search（）</span></span></code></pre><div class="line-numbers-wrapper" aria-hidden="true"><span class="line-number">1</span><br><span class="line-number">2</span><br><span class="line-number">3</span><br></div></div><br><p>首先，我们回到 page 内新建一个 main 文件，表示主页。创建之后，新建一个 class 类，这个类里面有 search 功能。search 允许我们搜一个关键字，所以需要传参，我将参数命名为 keyword。 搜完 keyword 之后会返回搜索结果页，所以需要 return 一个对象，我们假设 return 一个 search，而此时 search 还没有实现，因此我们需要要创建它。</p><br><div class="language- vp-adaptive-theme line-numbers-mode"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki github-dark vp-code-dark"><code><span class="line"><span style="color:#E1E4E8;">class Search：</span></span>
...
<span class="line"><span style="color:#24292E;">        return self</span></span></code></pre><div class="line-numbers-wrapper" aria-hidden="true"><span class="line-number">1</span><br><span class="line-number">2</span><br><span class="line-number">3</span><br></div></div><br><p>现在我们开始创建 search。 因为 PO 是一个类，所以我们可以使用一个类的方法去创建它。</p><br><p>创建的类有几个方法，第 1 个方法叫 search_by，它可以根据一些特征来进行搜索。 搜索特征有哪些呢？根据我们刚才的界面你可以看到，除了搜索普通关键词之外还有很多选项，比如根据发帖人或分类进行搜索。</p><br><p>这次我根据作者进行搜索。首先我们要给作者一个默认值 None。搜索完成之后仍然是当前的结果页，注意，这个时候它也是 PO，只不过 return 的是它自己self。</p><br><div class="language- vp-adaptive-theme line-numbers-mode"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki github-dark vp-code-dark"><code><span class="line"><span style="color:#E1E4E8;">class Search:</span></span>
<span class="line"><span style="color:#E1E4E8;">    def search_by(self,author=None):</span></span>
...
<span class="line"><span style="color:#24292E;">        return []</span></span></code></pre><div class="line-numbers-wrapper" aria-hidden="true"><span class="line-number">1</span><br><span class="line-number">2</span><br><span class="line-number">3</span><br><span class="line-number">4</span><br><span class="line-number">5</span><br><span class="line-number">6</span><br><span class="line-number">7</span><br><span class="line-number">8</span><br><span class="line-number">9</span><br></div></div><br><p>搜索完成后需要对结果进行判断，这时我们需要有 API 能够获取所有搜索结果中的作者。在这里，我们先获取搜索结果 get_results，搜索结果要 return 一个数组。然后是 get_authors，它要 return 一个字符串数组，用于我们后期的断言。</p><br><p>接着我们完善下类的导入，PO 模式基本上就已经成型了。</p><h2 id="编写测试用例" tabindex="-1">编写测试用例 <a class="header-anchor" href="#编写测试用例" aria-label="Permalink to &quot;编写测试用例&quot;">​</a></h2><p>我们开始编写测试用例。首先创建一个 test_search 的测试用例。</p><br><div class="language- vp-adaptive-theme line-numbers-mode"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki github-dark vp-code-dark"><code><span class="line"><span style="color:#E1E4E8;">class TestSearch:</span></span>
...
<span class="line"><span style="color:#24292E;">            assert result ==&quot;Wayyt&quot;</span></span></code></pre><div class="line-numbers-wrapper" aria-hidden="true"><span class="line-number">1</span><br><span class="line-number">2</span><br><span class="line-number">3</span><br><span class="line-number">4</span><br><span class="line-number">5</span><br><span class="line-number">6</span><br><span class="line-number">7</span><br><span class="line-number">8</span><br></div></div><br><p>第一步是创建 Setup，Setup 中包含初始化内容，比如首页 Main page 初始化，首先需要导入 Main，初始化后调用快捷搜索方法，这里输入 selenium，搜索完之后才能进入结果页。在这里我们创建一个 search_page 变量，用于保存结果页，这样就完成了对搜索结果页的初始化。</p><br><p>接着是 test_search，我们这次搜索一个简单的功能，使用 self.search_page 的 search_by 功能去搜作者，也可以搜索其他内容，这里我们搜索 wayyt 这个作者。</p><br><p>当然 search_by 也支持搜索结果内容，所以这里还可以追加更多的参数，比如 keyword。</p><br><p>这里令 keyword 参数为 selenium 并根据作者进行搜索，所以我希望获取的结果是作者。获取结果后存储进来，并开始编写断言，比如说断言搜索的内容，第 1 个断言是 assert 获取的作者长度至少要大于 0。 接下来对其中每个作者进行断言， 令 assert 的内容等于我刚才所写的 wayyt，这样就编写出来了一个简单的测试用例，这个测试用例它很好地描述了我的业务：根据关键字进行搜索，搜完之后对结果进行比对。</p><br><p>我们运行一下 case，case 一开始是报错的，这属于正常情况，因为我们还没有做自动化。</p><br><p>从这一步开始，我们已经设计好整个业务的用例：case 构建没问题了，测试用例的构建和 PO 的模式构建已经非常顺畅了。</p><h2 id="实现-po" tabindex="-1">实现 PO <a class="header-anchor" href="#实现-po" aria-label="Permalink to &quot;实现 PO&quot;">​</a></h2><p>接下来我们要完成对 PO 的实现。 首先回到之前的 case，把代码整合进来。</p><br><div class="language- vp-adaptive-theme line-numbers-mode"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki github-dark vp-code-dark"><code><span class="line"><span style="color:#E1E4E8;">class Main:</span></span>
...
<span class="line"></span>
<span class="line"><span style="color:#24292E;">        return Search(self.driver</span></span></code></pre><div class="line-numbers-wrapper" aria-hidden="true"><span class="line-number">1</span><br><span class="line-number">2</span><br><span class="line-number">3</span><br><span class="line-number">4</span><br><span class="line-number">5</span><br><span class="line-number">6</span><br><span class="line-number">7</span><br><span class="line-number">8</span><br><span class="line-number">9</span><br><span class="line-number">10</span><br><span class="line-number">11</span><br><span class="line-number">12</span><br><span class="line-number">13</span><br><span class="line-number">14</span><br><span class="line-number">15</span><br></div></div><br><p>Search类也需要用到driver来完成自己的自动化，所以我们给Search类增加一个可以传递driver的初始化方法。</p><br><div class="language- vp-adaptive-theme line-numbers-mode"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki github-dark vp-code-dark"><code><span class="line"><span style="color:#E1E4E8;">class Search:</span></span>
...
<span class="line"><span style="color:#24292E;">        self.driver=driver</span></span></code></pre><div class="line-numbers-wrapper" aria-hidden="true"><span class="line-number">1</span><br><span class="line-number">2</span><br><span class="line-number">3</span><br></div></div><br><p>我们对Search类也进行改造，完成对应方法的实现</p><div class="language- vp-adaptive-theme line-numbers-mode"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki github-dark vp-code-dark"><code><span class="line"><span style="color:#E1E4E8;">class Search:</span></span>
<span class="line"><span style="color:#E1E4E8;">    def __init__(self, driver):</span></span>
