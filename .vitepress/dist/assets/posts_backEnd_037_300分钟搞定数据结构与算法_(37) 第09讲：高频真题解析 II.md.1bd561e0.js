import{_ as e,j as o,o as t,g as c,k as a,h as l,s,Q as p}from"./chunks/framework.b3d8e22e.js";const V=JSON.parse('{"title":"例题分析一","description":"","frontmatter":{},"headers":[],"relativePath":"posts/backEnd/037_300分钟搞定数据结构与算法/(37) 第09讲：高频真题解析 II.md","filePath":"posts/backEnd/037_300分钟搞定数据结构与算法/(37) 第09讲：高频真题解析 II.md","lastUpdated":1696417798000}'),i={name:"posts/backEnd/037_300分钟搞定数据结构与算法/(37) 第09讲：高频真题解析 II.md"},r=p('<p>这节课继续学习另外三种高频题：</p><ul><li><p>合并区间和无重叠区间</p></li><li><p>火星字典</p></li><li><p>基本计算器</p></li></ul><h1 id="例题分析一" tabindex="-1">例题分析一 <a class="header-anchor" href="#例题分析一" aria-label="Permalink to &quot;例题分析一&quot;">​</a></h1><p>LeetCode 第 56 题：给出一个区间的集合，请合并所有重叠的区间。</p><p><strong>示例 1</strong></p><p>输入: [[1,3], [2,6], [8,10], [15,18]]</p><p>输出: [[1,6], [8,10], [15,18]]</p><p>解释: 区间 [1,3] 和 [2,6] 重叠，将它们合并为 [1,6]。</p><p><strong>示例 2</strong></p><p>输入: [[1,4], [4,5]]</p><p>输出: [[1,5]]</p><p>解释: 区间 [1,4] 和 [4,5] 可被视为重叠区间。</p><h6 id="解题思路-贪婪法" tabindex="-1">解题思路：贪婪法 <a class="header-anchor" href="#解题思路-贪婪法" aria-label="Permalink to &quot;解题思路：贪婪法&quot;">​</a></h6><p>在分析一些比较复杂的问题时，可以从比较简单的情况着手来寻找突破口，先来看看两个区间会出现多少种情况。</p><p>假设有区间 a 和 b，区间 a 的起始时间要早于 b 的起始时间。那么它们之间有如下 3 种可能会出现的情况。</p>',15),E=s("ol",null,[s("li",null,[s("p",null,"情况一：两个区间没有任何重叠的部分，因此区间不会发生融合。")]),s("li",null,[s("p",null,"情况二和三：区间有重叠。"),s("ol",null,[s("li",null,[s("p",null,"新区间的起始时间是 a 的起始时间，这个不变；")]),s("li",null,[s("p",null,"新区间的终止时间是 a 的终止时间和 b 的终止时间的最大值，这个就是融合两个区间的最基本的思想。")])])])],-1),y=p(`<ol><li><p>先将所有的区间按照起始时间的先后顺序排序，从头到尾扫描一遍</p></li><li><p>定义两个变量 previous 和 current，分别表示前一个区间和当前的区间</p><ol><li><p>如果没有融合，那么当前区间就变成了新的前一个区间，下一个区间成为新的当前区间</p></li><li><p>如果发生了融合，更新前一个区间的结束时间。</p></li></ol></li></ol><br><p>这个就是贪婪算法。</p><br><p><strong>代码实现</strong></p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki github-dark vp-code-dark"><code><span class="line"><span style="color:#E1E4E8;">int[][] merge(int[][] intervals) {</span></span>
...
<span class="line"></span>
<span class="line"><span style="color:#24292E;">}</span></span></code></pre></div><h6 id="解题思路二-贪婪法" tabindex="-1">解题思路二：贪婪法 <a class="header-anchor" href="#解题思路二-贪婪法" aria-label="Permalink to &quot;解题思路二：贪婪法&quot;">​</a></h6><h3 id="按照起始时间排序" tabindex="-1">按照起始时间排序 <a class="header-anchor" href="#按照起始时间排序" aria-label="Permalink to &quot;按照起始时间排序&quot;">​</a></h3><p><strong>举例</strong>：有四个区间 A，B，C，D，A 跨度很大，B 和 C 重叠，C 和 D 重叠，而 B 和 D 不重叠。</p><p><strong>解法</strong>：要尽可能少得删除区间，那么当遇到了重叠的时候，应该把区间跨度大，即结束比较晚的那个区间删除。因为如果不删除它，它会和剩下的其他区间发生重叠的可能性非常大。</p><p>当发现 A 和 B 重叠，如果不删除 A，就得牺牲 B，C，D，而正确的答案是只需要删除 A 和 C 即可。</p><p>按照上述思想求解，实现过程如下。</p><br><ol><li>A 和 B 重叠，由于 A 结束得比较晚，此时删除区间 A，保留区间 B。</li></ol>`,10),g=s("ol",{start:"2"},[s("li",null,"B 和 C 重叠，由于 C 结束得晚，把区间 C 删除，保留区间 B。")],-1),f=s("ol",{start:"3"},[s("li",null,"B 和 D 不重叠，结束，一共只删除了 2 个区间。")],-1),m=p(`<p><strong>代码实现</strong></p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki github-dark vp-code-dark"><code><span class="line"><span style="color:#E1E4E8;">int eraseOverlapIntervals(int[][] intervals) {</span></span>
...
<span class="line"><span style="color:#24292E;">rftt</span></span></code></pre></div><p>解法：</p><ol><li>比较以 w 开头的字符串，它们是 wrt 和 wrf，之所以 wrt 会排在 wrf 之前，是因为 t 比 f 在火星字典里出现的顺序要早。此时将这两个字母的关系表达为 t -&gt; f。</li></ol>`,34),k=s("ol",{start:"2"},[s("li",null,"比较 wrf 和 er，第一个字母开始不同，因此，得出 w 排在 e 之前，记为 w -> e。")],-1),_=s("ol",{start:"3"},[s("li",null,"比较 er 和 ett，从第二个字母开始不一样，因此，得出 r 排在 t 之前，记为 r -> t。")],-1),A=s("ol",{start:"4"},[s("li",null,"比较 ett 和 rftt，从第一个字母开始不一样，得出 e 排在 r 之前，记为 e -> r。")],-1),C=s("p",null,"梳理上述关系，得 t -> f，w -> e，r -> t，e -> r",-1),q=p(`<p><strong>代码实现</strong></p><p>包括两大步骤，第一步是根据输入构建一个有向图；第二步是对这个有向图进行拓扑排序。</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki github-dark vp-code-dark"><code><span class="line"><span style="color:#E1E4E8;">// 基本情况处理，比如输入为空，或者输入的字符串只有一个</span></span>
...
<span class="line"><span style="color:#24292E;">    }</span></span>
<span class="line"><span style="color:#24292E;">    ...</span></span>
<span class="line"><span style="color:#24292E;">}</span></span></code></pre></div><p>如上，在添加到优先队列的时候，过滤到那些空格就好了。</p><h6 id="解题思路二-引入减号" tabindex="-1">解题思路二：引入减号 <a class="header-anchor" href="#解题思路二-引入减号" aria-label="Permalink to &quot;解题思路二：引入减号&quot;">​</a></h6><p>例题：若表达式支持减法，应该怎么处理？例如：1 + 2 - 10。</p><p>解法 1：借助两个 stack，一个 stack 专门用来放数字；一个 stack 专门用来放符号。</p><p>解法 2：将表达式看作 1 + 2 + (-10)，把 -10 看成一个整体，同时，利用一个变量 sign 来表示该数字前的符号，这样即可沿用解法。</p>`,27),w=p(`<h3 id="代码实现-2" tabindex="-1">代码实现 <a class="header-anchor" href="#代码实现-2" aria-label="Permalink to &quot;代码实现&quot;">​</a></h3><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki github-dark vp-code-dark"><code><span class="line"><span style="color:#E1E4E8;">int calculate(String s) {   </span></span>
<span class="line"><span style="color:#E1E4E8;">    Queue&lt;</span><span style="color:#FDAEB7;font-style:italic;">Character</span><span style="color:#E1E4E8;">&gt; queue = new LinkedList</span><span style="color:#FDAEB7;font-style:italic;">&lt;</span><span style="color:#E1E4E8;">&gt;();</span></span>
<span class="line"><span style="color:#E1E4E8;">    for (char c : s.toCharArray()) {</span></span>
<span class="line"><span style="color:#E1E4E8;">        if (c != &#39; &#39;) queue.offer(c);</span></span>
<span class="line"><span style="color:#E1E4E8;">    }</span></span>
...
<span class="line"></span>
<span class="line"><span style="color:#24292E;">    return sum;</span></span>
<span class="line"><span style="color:#24292E;">}</span></span></code></pre></div><h6 id="解题思路三-引入乘除" tabindex="-1">解题思路三：引入乘除 <a class="header-anchor" href="#解题思路三-引入乘除" aria-label="Permalink to &quot;解题思路三：引入乘除&quot;">​</a></h6><p>例题：若引入乘法和除法，如何处理？举个例子：1 + 2 x 10。</p><br>`,5),S=p(`<h3 id="代码实现-3" tabindex="-1">代码实现 <a class="header-anchor" href="#代码实现-3" aria-label="Permalink to &quot;代码实现&quot;">​</a></h3><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki github-dark vp-code-dark"><code><span class="line"><span style="color:#E1E4E8;">int calculate(String s) {   </span></span>
<span class="line"><span style="color:#E1E4E8;">    Queue&lt;</span><span style="color:#FDAEB7;font-style:italic;">Character</span><span style="color:#E1E4E8;">&gt; queue = new LinkedList</span><span style="color:#FDAEB7;font-style:italic;">&lt;</span><span style="color:#E1E4E8;">&gt;();</span></span>
<span class="line"><span style="color:#E1E4E8;">    for (char c : s.toCharArray()) {</span></span>
<span class="line"><span style="color:#E1E4E8;">        if (c != &#39; &#39;) queue.offer(c);</span></span>
<span class="line"><span style="color:#E1E4E8;">    }</span></span>
