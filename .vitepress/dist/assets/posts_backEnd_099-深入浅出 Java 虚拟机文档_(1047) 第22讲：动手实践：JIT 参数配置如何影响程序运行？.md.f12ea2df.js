...
<span class="line"><span style="color:#24292E;">}</span></span></code></pre></div><br><p>方法内联的过程是非常智能的，内联后的代码，会按照一定规则进行再次优化。最终的机器码，在保证逻辑正确的前提下，可能和我们推理的完全不一样。在非常小的概率下，JIT 会出现 Bug，这时候可以关闭问题方法的内联，或者直接关闭 JIT 的优化，保持解释执行。实际上，这种 Bug 我从来没碰到过。</p><br><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki github-dark vp-code-dark"><code><span class="line"><span style="color:#E1E4E8;">-XX:CompileCommand=exclude,com/lagou/Test,test</span></span></code></pre><pre class="shiki github-light vp-code-light"><code><span class="line"><span style="color:#24292E;">-XX:CompileCommand=exclude,com/lagou/Test,test</span></span></code></pre></div><br><p>上面的参数，表示 com.lagou.Test 的 test 方法将不会进行 JIT 编译，一直解释执行。</p><br><p>另外，C2 支持的内联层次不超过 9 层，太高的话，CodeCache 区域会被挤爆，这个阈值可以通过 -XX:MaxInlineLevel 进行调整。相似的，编译后的代码超过一定大小也不会再内联，这个参数由 -XX:InlineSmallCode 进行调整。</p><br><p>有非常多的参数，被用来控制对内联方法的选择，整体来说，短小精悍的小方法更容易被优化。</p><br><p>这和我们在日常中的编码要求是一致的：代码块精简，逻辑清晰的代码，更容易获得优化的空间。</p><br>`,45),X=l(`<br><p>我们使用 JITWatch 再看一下对于 getA() 方法的调用，将鼠标悬浮在字节码指令上，可以看到方法已经被内联了。</p><h2 id="逃逸分析" tabindex="-1">逃逸分析 <a class="header-anchor" href="#逃逸分析" aria-label="Permalink to &quot;逃逸分析&quot;">​</a></h2><p>逃逸分析（Escape Analysis）是目前 JVM 中比较前沿的优化技术。通过逃逸分析，JVM 能够分析出一个新的对象使用范围，从而决定是否要将这个对象分配到堆上。</p><br><p>使用 -XX:+DoEscapeAnalysis 参数可以开启逃逸分析，逃逸分析现在是 JVM 的默认行为，这个参数可以忽略。</p><br><p>JVM 判断新创建的对象是否逃逸的依据有：</p><ul><li><p>对象被赋值给堆中对象的字段和类的静态变量；</p></li><li><p>对象被传进了不确定的代码中去运行。</p></li></ul><br><p>举个例子，在代码 1 中，虽然 map 是一个局部变量，但是它通过 return 语句返回，其他外部方法可能会使用它，这就是方法逃逸。另外，如果被其他线程引用或者赋值，则成为线程逃逸。</p><br><p>代码 2，用完 Map 之后就直接销毁了，我们就可以说 map 对象没有逃逸。</p><br><p>代码1：</p><br><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki github-dark vp-code-dark"><code><span class="line"><span style="color:#E1E4E8;">public Map fig(){</span></span>
...
<span class="line"><span style="color:#24292E;">        byte first = str[0];</span></span></code></pre></div><br>`,19),M=l('<br><p>JDK 中这种方法有接近 400 个，可以在 IDEA 中使用 <strong>Find Usages</strong> 找到它们。</p><h2 id="小结" tabindex="-1">小结 <a class="header-anchor" href="#小结" aria-label="Permalink to &quot;小结&quot;">​</a></h2><p>JIT 是现代 JVM 主要的优化点，能够显著地增加程序的执行效率，从解释执行到最高层次的 C2，一个数量级的性能提升也是有可能的。但即时编译的过程是非常缓慢的，耗时间也费空间，所以这些优化操作会和解释执行同时进行。</p><br><p>一般，方法首先会被解释执行，然后被 3 层的 C1 编译，最后被 4 层的 C2 编译，这个过程也不是一蹴而就的。</p><br><p>常用的优化手段，有公共子表达式消除、数组范围检查消除、方法内联、逃逸分析等。</p><br><p>其中，方法内联通过将短小精悍的代码融入到调用方法的执行逻辑里，来减少方法调用上的开支；逃逸分析通过分析变量的引用范围，对象可能会使用栈上分配的方式来减少 GC 的压力，或者使用标量替换来获取更多的优化。</p><br><p>这个过程的执行细节并不是那么&quot;确定&quot;，在不同的 JVM 中，甚至在不同的 HotSpot 版本中，效果也不尽相同。</p><br><p>使用 JITWatch 工具，能够看到字节码和机器码的对应关系，以及执行过程中的一系列优化操作。若想要了解这个工具的更多功能，<a href="https://github.com/AdoptOpenJDK/jitwatch/wiki" target="_blank" rel="noreferrer">可以点击这里参考 wiki</a>。</p>',14);function V(j,D,x,P,q,O){const n=e("Image");return t(),o("div",null,[c,a(n,{alt:"",src:"https://s0.lgstatic.com/i/image3/M01/7B/89/Cgq2xl57A7qAGBF6AAAx8j1m3Kw525.jpg"}),r,a(n,{alt:"",src:"https://s0.lgstatic.com/i/image3/M01/02/73/Ciqah157A7uAPt49AAD7831CinU309.jpg"}),E,d,y,a(n,{alt:"",src:"https://s0.lgstatic.com/i/image3/M01/7B/89/Cgq2xl57A7uAf1o-AAEJfdrafg8749.jpg"}),g,u,h,b,_,a(n,{alt:"",src:"https://s0.lgstatic.com/i/image3/M01/02/73/Ciqah157A7uAB8iPAAFu7HBxG4w040.jpg"}),C,m,v,I,T,a(n,{alt:"",src:"https://s0.lgstatic.com/i/image3/M01/7B/89/Cgq2xl57A7uAO1CdAACX4fWefMo181.jpg"}),A,f,J,a(n,{alt:"",src:"https://s0.lgstatic.com/i/image3/M01/7B/89/Cgq2xl57A7yAZvChAAG-RAna12A372.jpg"}),k,a(n,{alt:"",src:"https://s0.lgstatic.com/i/image3/M01/02/73/Ciqah157A7yAJiHeAAF5gyjxA3w172.jpg"}),X,a(n,{alt:"",src:"https://s0.lgstatic.com/i/image3/M01/7B/89/Cgq2xl57A7yAZKc2AAFu39pb5SE629.jpg"}),S,a(n,{alt:"",src:"https://s0.lgstatic.com/i/image3/M01/02/73/Ciqah157A7yAHKkqAADmapAcCaE181.jpg"}),M])}const w=p(i,[["render",V]]);export{H as __pageData,w as default};
