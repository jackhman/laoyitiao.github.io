...
<span class="line"><span style="color:#24292E;">      63</span></span></code></pre></div><p>分析栈信息，你可能会直接查找 locked 关键字，如下图所示，但是这样的方法一般没什么用，我们需要做更多的统计。</p><br>`,6),u=l("br",null,null,-1),b=l("p",null,"注意下图中有一个处于 BLOCKED 状态的线程，它阻塞在对锁的获取上（wating to lock）。大体浏览一下 DUMP 文件，会发现多处这种状态的线程，可以使用如下脚本进行统计。",-1),h=l("br",null,null,-1),v=n(`<br><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki github-dark vp-code-dark"><code><span class="line"><span style="color:#E1E4E8;">cat 10271.tdump| grep &quot;waiting to lock &quot; | awk &#39;{print $5}&#39; | sort | uniq -c | sort -k1 -r</span></span>
...
<span class="line"><span style="color:#24292E;">   1 &lt;</span><span style="color:#B31D28;font-style:italic;">0x0000000782dfdae0</span><span style="color:#24292E;">&gt;</span></span></code></pre></div><br><p>我们找到给 <strong>0x0000000782e1b590</strong>上锁的执行栈，可以发现全部卡在了 HttpClient 的读操作上。在实际场景中，可以看下排行比较靠前的几个锁地址，找一下共性。</p><br>`,5),m=n(`<br><p>返回头去再看一下代码。我们发现 HttpClient 是共用了一个连接池，当连接数超过 100 的时候，就会阻塞等待。它的连接超时时间是 10 秒，这和 slow 接口的耗时不相上下。</p><br><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki github-dark vp-code-dark"><code><span class="line"><span style="color:#E1E4E8;">private final static HttpConnectionManager httpConnectionManager = new SimpleHttpConnectionManager(true);</span></span>
...
<span class="line"><span style="color:#24292E;">            o.wait();</span></span>
<span class="line"><span style="color:#24292E;">        }</span></span>
<span class="line"><span style="color:#24292E;">    }</span></span></code></pre></div><br><p>说明在获得了监视器之后，又调用了 java.lang.Object.wait() 方法。</p><br><p>关于这部分的原理，可以参见一张经典的图。每个监视器（Monitor）在某个时刻，只能被一个线程拥有，该线程就是&quot;Active Thread&quot;，而其他线程都是&quot;Waiting Thread&quot;，分别在两个队列&quot;Entry Set&quot;和&quot;Wait Set&quot;里面等候。在&quot;Entry Set&quot;中等待的线程状态是&quot;Waiting for monitor entry&quot;，而在&quot;Wait Set&quot;中等待的线程状态是&quot;in Object.wait()&quot;。</p><br>`,48),k=n(`<br><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki github-dark vp-code-dark"><code><span class="line"><span style="color:#E1E4E8;">&quot;wait-demo&quot; #12 prio=5 os_prio=31 cpu=0.14ms elapsed=12.58s tid=0x00007fb66609e000 nid=0x6103 in Object.wait()  [0x000070000f2bd000]</span></span>
...
<span class="line"><span style="color:#24292E;">    at java.lang.Thread.run(java.base@13.0.1/Thread.java:830)</span></span>
<span class="line"></span>
<span class="line"><span style="color:#24292E;">Found 1 deadlock</span></span></code></pre></div><br><br><p>当然，关于线程的 dump，也有一些线上分析工具可以使用。下图是 <a href="https://fastthread.io/" target="_blank" rel="noreferrer">fastthread</a> 的一个分析结果，但也需要你先了解这些情况发生的意义。</p><br>`,14),j=n('<h2 id="小结" tabindex="-1">小结 <a class="header-anchor" href="#小结" aria-label="Permalink to &quot;小结&quot;">​</a></h2><p>本课时主要介绍了一个处处有问题的报表系统，并逐步解决了它的 OOM 问题，同时定位到了拒绝服务的原因。</p><br><p>在研发资源不足的时候，我们简单粗暴的进行了硬件升级，并切换到了更加优秀的 G1 垃圾回收器，还通过代码手段进行了问题的根本解决：</p><ul><li><p>缩减查询的字段，减少常驻内存的数据；</p></li><li><p>去掉不必要的、命中率低的堆内缓存，改为分布式缓存；</p></li><li><p>从产品层面限制了单次请求对内存的无限制使用。</p></li></ul><br><p>在这个过程中，使用 MAT 分析堆数据进行问题代码定位，帮了大忙。代码优化的手段是最有效的，改造完毕后，可以节省更多的硬件资源。事实上，使用了 G1 垃圾回收器之后，那些乱七八糟的调优参数越来越少用了。</p><br><p>接下来，我们使用 jstack 分析了一个出现频率非常非常高的问题，主要是不同速度的接口在同一应用中的资源竞争问题，我们发现一些成熟的微服务框架，都会对这些资源进行限制和隔离。</p><br><p>最后，以 4 个简单的示例，展示了 jstack 输出内容的一些意义。代码都在 git 仓库里，你可以实际操作一下，希望对你有所帮助。</p>',11);function _(f,C,T,S,x,w){const s=e("Image");return t(),o("div",null,[i,a(s,{alt:"",src:"https://s0.lgstatic.com/i/image3/M01/6B/C0/CgpOIF5YswCAdZ7JAABhTXSnBAQ829.jpg"}),r,a(s,{alt:"",src:"https://s0.lgstatic.com/i/image3/M01/6B/C0/Cgq2xl5YswGAHr8VAAA1xZ7pDg8510.jpg"}),E,a(s,{alt:"",src:"https://s0.lgstatic.com/i/image3/M01/6B/C0/CgpOIF5YswGAC8SRAAEG1hWaI_I736.jpg"}),d,a(s,{alt:"",src:"https://s0.lgstatic.com/i/image3/M01/6B/C0/Cgq2xl5YswGAexkRAABvM5JegWw758.jpg"}),y,a(s,{alt:"",src:"https://s0.lgstatic.com/i/image3/M01/6B/C0/CgpOIF5YswGAJ5P7AAFAbNc33Jo615.jpg"}),g,a(s,{alt:"",src:"https://s0.lgstatic.com/i/image3/M01/6B/C0/Cgq2xl5YswGAXVAQAACZ2MJUwQ4559.jpg"}),u,b,h,a(s,{alt:"",src:"https://s0.lgstatic.com/i/image3/M01/6B/C0/CgpOIF5YswKACEi1AAFlz8kljfk883.jpg"}),v,a(s,{alt:"",src:"https://s0.lgstatic.com/i/image3/M01/6B/C0/Cgq2xl5YswKAFuC5AAH6vr-FsN8996.jpg"}),m,a(s,{alt:"",src:"https://s0.lgstatic.com/i/image3/M01/6B/C0/CgpOIF5YswKAae_EAABYAiurKBQ445.jpg"}),k,a(s,{alt:"",src:"https://s0.lgstatic.com/i/image3/M01/6B/C0/Cgq2xl5YswOALstRAAKRsvw-7ZU685.jpg"}),j])}const q=p(c,[["render",_]]);export{D as __pageData,q as default};
