import{_ as o,j as e,o as c,g as t,k as p,h as n,s,Q as l}from"./chunks/framework.4e7d56ce.js";const R=JSON.parse('{"title":"案例集成：ReactiveSpringCSS 中的消息消费场景 ","description":"","frontmatter":{},"headers":[],"relativePath":"posts/backEnd/Spring 响应式编程实战_文档/(7002) 20  消息消费：如何选择可用的高级开发技巧？.md","filePath":"posts/backEnd/Spring 响应式编程实战_文档/(7002) 20  消息消费：如何选择可用的高级开发技巧？.md","lastUpdated":1696417798000}'),r={name:"posts/backEnd/Spring 响应式编程实战_文档/(7002) 20  消息消费：如何选择可用的高级开发技巧？.md"},i=s("p",null,"在上一讲中，我们讨论了 ReactiveSpringCSS 案例中基于 Reactive Spring Cloud Stream 的消息发布场景以及实现方式。今天我将延续上一讲的内容，为你介绍消息消费的应用场景，具体讲解如何在服务中添加消息消费者，以及使用各项消息消费的高级开发技巧。",-1),E=s("h3",{id:"案例集成-reactivespringcss-中的消息消费场景",tabindex:"-1"},[n("案例集成：ReactiveSpringCSS 中的消息消费场景 "),s("a",{class:"header-anchor",href:"#案例集成-reactivespringcss-中的消息消费场景","aria-label":'Permalink to "案例集成：ReactiveSpringCSS 中的消息消费场景"'},"​")],-1),y=s("p",null,"我们继续讨论 ReactiveSpringCSS 案例，根据整个消息交互流程，customer-service 就是 AccountChangedEvent 事件的消费者。根据上一讲讨论的交互流程，customer-service 需要把变更后的用户账户信息更新到 Redis 缓存中。",-1),u=s("p",null,"在 Spring Cloud Stream 中，负责消费消息的是 Sink 组件。因此，我们同样围绕 AccountChangedEvent 事件研究 customer -service 内部的整个实现流程，如下图所示。",-1),d=s("p",null,"customer-service 消息消费实现流程图",-1),g=s("p",null,"在上图中，AccountChangedEvent 事件通过消息中间件发送到 Reactive Spring Cloud Stream 中，Reactive Spring Cloud Stream 通过 Sink 获取消息并交由 ReactiveAccountChangedSink 实现具体的消费逻辑。结合前面提到的消息消费场景下的缓存处理需求，可以想象这个 ReactiveAccountChangedSink 会负责实现缓存相关的处理逻辑。",-1),h=s("p",null,'让我们把消息消费过程与 customer-service 中的业务流程串联起来。我们知道在 customer-service 中存在 ReactiveAccountClient 类，它通过判断缓存中是否存储目标用户账户对象来决定是否需要发起远程调用。我们已经在"16 | Redis 集成：如何实现对 Redis 的响应式数据访问"中构建了 Redis 缓存服务和 ReactiveAccountClient，你可以回顾一下。',-1),C=s("p",null,"下图展示了采用这一设计思想之后的流程。",-1),b=l(`<p>用户账户更新流程图</p><p>在上图中，我们看到 account-service 异步发送的 AccountChangedEvent 事件会被 ReactiveAccountChangedSink 所消费，然后 ReactiveAccountChangedSink 将更新后的用户账户信息存储到缓存以供 customer-service 使用。显然，ReactiveAccountChangedSink 是整个流程的关键。如何实现这个 Sink，就是这一讲的主要内容。</p><h3 id="构建响应式-sink-组件" tabindex="-1">构建响应式 Sink 组件 <a class="header-anchor" href="#构建响应式-sink-组件" aria-label="Permalink to &quot;构建响应式 Sink 组件&quot;">​</a></h3><p>针对消费者组件，我们采用和消息发布者相同的方式进行实现。首先要说的还是使用 @EnableBinding 注解来初始化消息消费者。</p><h4 id="使用-enablebinding-注解" tabindex="-1">使用 @EnableBinding 注解 <a class="header-anchor" href="#使用-enablebinding-注解" aria-label="Permalink to &quot;使用 @EnableBinding 注解&quot;">​</a></h4><p>与初始化消息发布环境一样，我们同样需要在 customer-service 中引入 spring-cloud-stream、spring-cloud-stream-reactive 以及 spring-cloud-starter-stream-rabbit 这几个 Maven 依赖，并构建 Bootstrap 类。customer-service 中的 Bootstrap 类是 CustomerApplication，其代码如下所示。</p><div class="language-java vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">java</span><pre class="shiki github-dark vp-code-dark"><code><span class="line"><span style="color:#E1E4E8;">@</span><span style="color:#F97583;">SpringCloudApplication</span></span>
...
<span class="line"><span style="color:#24292E;">      binders:</span></span>
<span class="line"><span style="color:#24292E;">      ...</span></span></code></pre></div><p>以上基于 RabbitMQ 的配置信息中，我们关注&quot;bindings&quot;段中的通道名称使用了自定义的&quot;accountChangedChannel&quot;，并且在该配置项中设置了&quot;group&quot;为&quot;customerGroup&quot;。</p><p>请注意，在分布式环境下，对于 RabbitMQ 等消息中间件而言，使用消费者分组与其说是一个可选性，倒不如说是一个必选项。因为只有采用消费者分组才能确保在集群环境下消息得到正确的消费。所以，在日常应用过程中，通常都建议你使用这项开发技巧。</p><h4 id="使用消息分区" tabindex="-1">使用消息分区 <a class="header-anchor" href="#使用消息分区" aria-label="Permalink to &quot;使用消息分区&quot;">​</a></h4><p>最后一项 Spring Cloud Stream 使用上的高级主题是使用消费分区。同样在集群环境下，假设存在两个 customer-service 实例，我们希望用户账户信息中 id 以数字结尾的 AccountChangedEvent 始终由第一个 customer-service 实例进行消费，而 id 以字母结尾的AccountChangedEvent 则始终由第二个 customer-service 实例进行消费。基于类似这样的需求，我们就可以构建消息分区，如下所示。</p>`,8),v=l(`<p>customer-service 消息分区效果示意图</p><p>要想实现上图所示的消息消费效果，我们唯一要做的事情还是重构 account-service 中 Binder 配置，如下所示。</p><div class="language-xml vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">xml</span><pre class="shiki github-dark vp-code-dark"><code><span class="line"><span style="color:#E1E4E8;">spring:</span></span>
<span class="line"><span style="color:#E1E4E8;">  cloud:</span></span>
<span class="line"><span style="color:#E1E4E8;">    stream:</span></span>
<span class="line"><span style="color:#E1E4E8;">      bindings:</span></span>
<span class="line"><span style="color:#E1E4E8;">        default:</span></span>
