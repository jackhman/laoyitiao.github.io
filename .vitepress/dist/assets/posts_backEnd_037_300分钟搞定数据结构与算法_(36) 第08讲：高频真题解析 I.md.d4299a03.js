...
<span class="line"><span style="color:#24292E;">}</span></span></code></pre></div><h6 id="解题思路三-优化的线性法" tabindex="-1">解题思路三：优化的线性法 <a class="header-anchor" href="#解题思路三-优化的线性法" aria-label="Permalink to &quot;解题思路三：优化的线性法&quot;">​</a></h6><p>在上述例题中，能否让慢指针不再一步一步地挪动，而是迅速地跳到字符 b 的位置？</p><p>可以用哈希表来记录每个字符以及它出现的位置，当遇到了字符 a 的时候，就知道跟它重复的前一个字符出现的位置，只需要让慢指针指向那个位置的下一个即可。（如果题目说所有字符都是字母的话，也可以用一个数组去记录。）</p>`,13),f=s("p",null,"注意：在运用这个算法的时候，不能去数哈希集合的元素个数来作为子串的长度，所以得额外维护一个变量来保存最后的结果。",-1),v=s("p",null,"但是在一些情况下，我们不能简单地将取出来的重复位置加 1，如下：快指针 j 指向的字符是 e，而 e 在哈希表里记录的位置是 1。",-1),A=p(`<p>在这种情况下，没有必要让 i 重新指向 e 后面的 a。此时，i 应该保留在原地不动。因此，i 被移动到的新位置应该等于 max(i，重复字符出现位置 + 1）。</p><p><strong>代码实现</strong></p><br><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki github-dark vp-code-dark"><code><span class="line"><span style="color:#E1E4E8;">// 定义一个哈希表用来记录上一次某个字符出现的位置，并初始化结果 max 为 0</span></span>
<span class="line"><span style="color:#E1E4E8;">int lengthOfLongestSubstring(String s) {</span></span>
...
<span class="line"><span style="color:#24292E;">    </span></span>
<span class="line"><span style="color:#24292E;">    return max;</span></span>
<span class="line"><span style="color:#24292E;">}</span></span></code></pre></div><h6 id="例题分析二" tabindex="-1">例题分析二 <a class="header-anchor" href="#例题分析二" aria-label="Permalink to &quot;例题分析二&quot;">​</a></h6><p>LeetCode 第 04 题：给定两个大小为 m 和 n 的有序数组 nums1 和 nums2。请你找出这两个有序数组的中位数，并且要求算法的时间复杂度为 O(log(m+n))。你可以假设 nums1 和 nums2 不会同时为空。</p><p><strong>示例1</strong></p><p>nums1 = [1, 3]</p><p>nums2 = [2]</p><p>则中位数是 2.0</p><p><strong>示例2</strong></p><p>nums1 = [1, 2]</p><p>nums2 = [3, 4]</p><p>则中位数是 (2 + 3)/2 = 2.5</p><h6 id="解题思路一-暴力法-1" tabindex="-1">解题思路一：暴力法 <a class="header-anchor" href="#解题思路一-暴力法-1" aria-label="Permalink to &quot;解题思路一：暴力法&quot;">​</a></h6><p>因为两个数组都是排好序的，可以利用归并排序将它们合并成一个长度为 m+n 的有序数组，合并的时间复杂度是 m+n，然后从中选取中位数，整体的时间复杂度就是 O(m+n)。</p><p>这是比较直观的解法，但是比题目要求的 O(log(m+n)) 慢了许多，并不适合。</p><h6 id="解题思路二-切分法" tabindex="-1">解题思路二：切分法 <a class="header-anchor" href="#解题思路二-切分法" aria-label="Permalink to &quot;解题思路二：切分法&quot;">​</a></h6><p>假设 m+n = L，若 L 为奇数，即两个数组的元素总个数为奇数，那么它们的中位数就是第 int(L / 2) + 1 小的数。例如，数组 { 1, 2, 3 } 的中位数是 2，2 就是第二小的数 2 = int(3/2) + 1。</p><p>如果 L 是偶数，那么中位数就是第 int(L/2) 小与第 int(L/2)+1 小的数的和的平均值。例如，数组 {1, 2, 3, 4} 的中位数是 (2 + 3) / 2 = 2.5，其中，2 = int(4/2)，3 = int(4/2) + 1。</p><p>因此这个问题就转变为在两个有序数组中寻找第 k 小的数 f(k)，当 L 是奇数的时候，另 k = L/2，结果为 f(k + 1)；而当 L 是偶数的时候，结果为 f(k) + f(k + 1) / 2。</p><p>如何从两个排好序的数组里找出第 k 小的数？</p><p>假设我们从第一个数组里前面 k~1~ 个数，从第二个数组里取出前面 k~2~ 个数，如下图。</p>`,23),w=s("p",null,"假设 k = 5，k~1~= 3，k~2~ = 2，有下面几种情况。",-1),C=s("ol",null,[s("li",null,"当 a~2~ = b~1~时，可以肯定 a~2~ 和 b~1~ 就是第 5 小的数。")],-1),q=s("p",null,"因为当把 a~0~、a~1~、a~2~ 以及 b~0~、b~1~ 按照大小顺序合并在一起的时候， a~2~ 和 b~1~ 一定排在最后面，完全不需要考虑 a~0~、a~1~ 和 b~0~ 的大小关系。其中一种可能的排列如下。",-1),j={start:"2"},x=s("p",null,"而最终第 5 小的数是 a~3~ 5 这个数。因此，在这种情况下，我们不能得出第 5 小的数是哪个。",-1),T=s("br",null,null,-1),L=s("p",null,"但是，在这种情况下，至少我们可以肯定的是，我们要找的结果肯定不会在 a~0~，a~1~，a~2~ 之间，即不会出现在 nums1 数组的前半段里。为什么呢？很简单，因为如果第 5 小的数是 a~0~，a~1~，a~2~ 其中一个的话，意味着 k~1~+k~2~ 必然大于 5，这就跟我们的假设不符了。",-1),D=s("p",null,"那么结果会不会在 nums2 的后半段呢？不可能，加入第 5 小的数在 nums2 的后半段，那么意味着，这个数要大于 b~1~（即 7），也会大于 a~2~（即 3），但是 k~1~ + k~2~ 已经等于 5了，所以就和假设冲突了。",-1),B=p("<p>在这样的情况下，我们可以把搜索的范围缩小，从 nums1 的后半段以及 nums2 的前半段中继续寻找。</p><ol><li>当 a~2~ &gt; b~1~ 的时候，无法肯定 a~2~ 和 b~1~ 是不是第 5 小的数。举例如下。</li></ol><p>nums1[] = {5, 6, <strong>7</strong>, 8, 9}</p><p>nums2[] = {1, <strong>2</strong>, 3, 4}</p><p>a~2~ = 7，b~1~ = 2</p><br><p>而最终第 5 小的数是 a~0~ 5 这个数。因此，在这种情况下，我们也不能得出第 5 小的数是哪个。</p><p>但是，在这种情况下，至少我们可以肯定的是，我们要找的结果肯定不会是 b0，或者</p><p>nums2 数组的前半段里。为什么呢？因为如果第 5 小的数是 b0 的话，意味着 k1+k2 必然大</p><p>于 5，这也跟我们的假设不符了。同样的，结果也不可能在 nums1 的后半段里。</p><br><p>在这样的情况下，我们可以把搜索的范围缩小，从 nums2 的后半段以及 nums1 中继续寻找。</p>",12),S=p(`<br><p><strong>代码实现</strong></p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki github-dark vp-code-dark"><code><span class="line"><span style="color:#E1E4E8;">double findMedianSortedArrays(int nums1[], int nums2[]) {</span></span>
...
<span class="line"><span style="color:#24292E;">  </span></span>
<span class="line"><span style="color:#24292E;">}</span></span></code></pre></div><h6 id="解题思路三-分治法" tabindex="-1">解题思路三：分治法 <a class="header-anchor" href="#解题思路三-分治法" aria-label="Permalink to &quot;解题思路三：分治法&quot;">​</a></h6><p>当 k=1 的时候，直接返回结果；当 k=2 的时候，把这两个链表归并。当 k=3 的时候，我们可以把它们分成两组，分别归并完毕后再进行最后的归并操作，如下。</p>`,13),U=p(`<p>上述做法运用了典型的分治思想，非常类似归并排序操作。</p><p><strong>代码实现</strong></p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki github-dark vp-code-dark"><code><span class="line"><span style="color:#E1E4E8;">public ListNode mergeKLists(ListNode[] lists, int low, int high) {</span></span>
...
<span class="line"><span style="color:#24292E;">}</span></span></code></pre></div><p>合并两个排好序的链表非常简单，此处使用递归函数，可以尝试非递归写法。</p><br><p><strong>时间复杂度</strong>：O(nk×log(k))。</p><p><strong>空间复杂度</strong>：O(1)。因为不像最小堆解法那样需要维护一个额外的数据结构。</p><p>提示：因为这道题针对的是链表，所以很多操作都直接在链表上进行。</p><h6 id="结语" tabindex="-1">结语 <a class="header-anchor" href="#结语" aria-label="Permalink to &quot;结语&quot;">​</a></h6><p>这节课剖析了三道非常经典的高频题，从不同的角度考虑解决问题的方案，并回顾了经典的快速选择算法。下节课将讨论另外三道经典的高频题。</p><br><br>`,12);function G(X,z,J,Z,$,ss){const n=t("Image");return o(),i("div",null,[r,s("h6",E,[a(n,{alt:"",src:"http://s0.lgstatic.com/i/image2/M01/91/50/CgotOV2InpGAXYErAIDbacLXGUw574.gif"}),l(),y]),u,a(n,{alt:"",src:"http://s0.lgstatic.com/i/image2/M01/91/30/CgoB5l2InpOAH6lGAE1uzx5xDe0090.gif"}),h,m,g,d,k,a(n,{alt:"",src:"http://s0.lgstatic.com/i/image2/M01/91/50/CgotOV2InpOAdm0bAAA91_bKAZI920.png"}),l(),b,a(n,{alt:"",src:"http://s0.lgstatic.com/i/image2/M01/91/50/CgotOV2InpSAJ2NcAEb7BX-6f9k727.gif"}),_,s("p",null,[l("遇到字符 a，此时哈希表的记录 {d: 0, e: 1, a: 2, b: 3: c: 4}，a 的位置是 2，把 2 加上 1 等于 3，就能让慢指针 i 指向下标为 3 的位置，即 b 字符的地方。 "),a(n,{alt:"",src:"http://s0.lgstatic.com/i/image2/M01/91/30/CgoB5l2InpWAAnvyAFQgtLp9Xgg419.gif"})]),f,v,a(n,{alt:"",src:"http://s0.lgstatic.com/i/image2/M01/91/50/CgotOV2InpaABE_PAACvFYyHH4E185.png"}),A,a(n,{alt:"",src:"http://s0.lgstatic.com/i/image2/M01/91/30/CgoB5l2InpaAZ6lZAAC7LqjZWnU769.png"}),w,C,q,a(n,{alt:"",src:"http://s0.lgstatic.com/i/image2/M01/91/50/CgotOV2InpaAAPVqAADiT5r6EoE952.png"}),l(),s("ol",j,[s("li",null,[l("当 a~2~ < b~1~ 的时候，无法肯定 a~2~ 和 b~1~ 是不是第 5 小的数。举例如下。 "),a(n,{alt:"",src:"http://s0.lgstatic.com/i/image2/M01/91/30/CgoB5l2InpaAdWQTAACcO4jiikI797.png"})])]),x,T,L,D,a(n,{alt:"",src:"http://s0.lgstatic.com/i/image2/M01/91/50/CgotOV2InpiANLV4AGB7zVnr6UQ076.gif"}),B,a(n,{alt:"",src:"http://s0.lgstatic.com/i/image2/M01/91/30/CgoB5l2InpmAHlttACt9vWodiQU791.gif"}),S,a(n,{alt:"",src:"http://s0.lgstatic.com/i/image2/M01/91/50/CgotOV2InpmAMCH-AACt9UBVjdM117.png"}),N,I,a(n,{alt:"",src:"http://s0.lgstatic.com/i/image2/M01/91/30/CgoB5l2InpqADa6YACUXrW-nt7c325.gif"}),O,P,M,V,a(n,{alt:"",src:"http://s0.lgstatic.com/i/image2/M01/91/50/CgotOV2InpuAQV-WAB7LzfFC6cc207.gif"}),K,a(n,{alt:"",src:"http://s0.lgstatic.com/i/image2/M01/91/50/CgotOV2InpyAWKEsACyLx0hVqIg915.gif"}),F,a(n,{alt:"",src:"http://s0.lgstatic.com/i/image2/M01/91/30/CgoB5l2Inp-ANdNWAFfawadYn4g058.gif"}),R,s("ol",null,[s("li",null,[s("p",null,[l("从 1万 个服务器中选择一个作为主机（master server）。这台主机将扮演主导快速选择算法的角色。"),a(n,{alt:"",src:"http://s0.lgstatic.com/i/image2/M01/91/30/CgoB5l2Inp-ADWduAACy1HlNYdU966.png"})])]),H]),a(n,{alt:"",src:"http://s0.lgstatic.com/i/image2/M01/91/50/CgotOV2InqCADiJBADbDhpQ_g34352.gif"}),Q,a(n,{alt:"",src:"http://s0.lgstatic.com/i/image2/M01/91/50/CgotOV2InqOAW68SAEJNyDb5D5o855.gif"}),W,s("p",null,[l("那么，第二小的如何选择？例如，有下面 k 个链表。 "),a(n,{alt:"",src:"http://s0.lgstatic.com/i/image2/M01/91/30/CgoB5l2InqSAHLY0ACSlxIOkYh0731.gif"})]),Y,a(n,{alt:"",src:"http://s0.lgstatic.com/i/image2/M01/91/50/CgotOV2InqWAExguACYe_inhacY345.gif"}),l(),U])}const ls=e(c,[["render",G]]);export{as as __pageData,ls as default};
