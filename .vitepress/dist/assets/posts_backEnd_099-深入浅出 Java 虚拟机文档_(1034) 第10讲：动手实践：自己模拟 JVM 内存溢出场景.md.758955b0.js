import{_ as e,D as l,o as t,g as o,J as n,h as p,Q as s}from"./chunks/framework.f67d7268.js";const S=JSON.parse('{"title":"第10讲：动手实践：自己模拟JVM内存溢出场景","description":"","frontmatter":{},"headers":[],"relativePath":"posts/backEnd/099-深入浅出 Java 虚拟机文档/(1034) 第10讲：动手实践：自己模拟 JVM 内存溢出场景.md","filePath":"posts/backEnd/099-深入浅出 Java 虚拟机文档/(1034) 第10讲：动手实践：自己模拟 JVM 内存溢出场景.md","lastUpdated":1696682708000}'),c={name:"posts/backEnd/099-深入浅出 Java 虚拟机文档/(1034) 第10讲：动手实践：自己模拟 JVM 内存溢出场景.md"},r=s(`<h1 id="第10讲-动手实践-自己模拟jvm内存溢出场景" tabindex="-1">第10讲：动手实践：自己模拟JVM内存溢出场景 <a class="header-anchor" href="#第10讲-动手实践-自己模拟jvm内存溢出场景" aria-label="Permalink to &quot;第10讲：动手实践：自己模拟JVM内存溢出场景&quot;">​</a></h1><p>本课时我们主要自己模拟一个 JVM 内存溢出的场景。在模拟 JVM 内存溢出之前我们先来看下这样的几个问题。</p><ul><li><p>老年代溢出为什么那么可怕？</p></li><li><p>元空间也有溢出？怎么优化？</p></li><li><p>如何配置栈大小？避免栈溢出？</p></li><li><p>进程突然死掉，没有留下任何信息时如何进行排查？</p></li></ul><p>年轻代由于有老年代的担保，一般在内存占满的时候，并没什么问题。但老年代满了就比较严重了，它没有其他的空间用来做担保，只能 OOM 了，也就是发生 Out Of Memery Error。JVM 会在这种情况下直接停止工作，是非常严重的后果。</p><br><p>OOM 一般是内存泄漏引起的，表现在 GC 日志里，一般情况下就是 GC 的时间变长了，而且每次回收的效果都非常一般。GC 后，堆内存的实际占用呈上升趋势。接下来，我们将模拟三种溢出场景，同时使用我们了解的工具进行观测。</p><br><p>在开始之前，请你下载并安装一个叫作 VisualVM 的工具，我们使用这个图形化的工具看一下溢出过程。</p><br><p>虽然 VisualVM 工具非常好用，但一般生产环境都没有这样的条件，所以大概率使用不了。新版本 JDK 把这个工具单独抽离了出去，需要自行下载。</p><br><p>这里需要注意下载安装完成之后请在插件选项中勾选 Visual GC 下载，它将可视化内存布局。</p><h2 id="堆溢出模拟" tabindex="-1">堆溢出模拟 <a class="header-anchor" href="#堆溢出模拟" aria-label="Permalink to &quot;堆溢出模拟&quot;">​</a></h2><p>首先，我们模拟堆溢出的情况，在模拟之前我们需要准备一份测试代码。这份代码开放了一个 HTTP 接口，当你触发它之后，将每秒钟生成 1MB 的数据。由于它和 GC Roots 的强关联性，每次都不能被回收。</p><br><p>程序通过 JMX，将在每一秒创建数据之后，输出一些内存区域的占用情况。然后通过访问 <a href="http://localhost:8888" target="_blank" rel="noreferrer">http://localhost:8888</a> 触发后，它将一直运行，直到堆溢出。</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki github-dark vp-code-dark"><code><span class="line"><span style="color:#E1E4E8;">import com.sun.net.httpserver.HttpContext;</span></span>
