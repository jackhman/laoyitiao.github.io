import{_ as l,j as p,o as t,g as o,k as n,s,h as r,Q as a}from"./chunks/framework.4e7d56ce.js";const x=JSON.parse('{"title":"树结构定义 ","description":"","frontmatter":{},"headers":[],"relativePath":"posts/devops/110-测试开发核心技术文档/(334) 第17讲：树结构及经典考题剖析.md","filePath":"posts/devops/110-测试开发核心技术文档/(334) 第17讲：树结构及经典考题剖析.md","lastUpdated":1696417798000}'),c={name:"posts/devops/110-测试开发核心技术文档/(334) 第17讲：树结构及经典考题剖析.md"},i=s("p",null,"本课时我们主要讲解树结构，以及剖析树结构相关的考题。",-1),E=s("h2",{id:"树结构定义",tabindex:"-1"},[r("树结构定义 "),s("a",{class:"header-anchor",href:"#树结构定义","aria-label":'Permalink to "树结构定义"'},"​")],-1),d=s("p",null,"首先，我们先来看下什么是树结构，树结构的定义是如果有一种数据，它既有自己的核心字段，同时又有指向它的若干个子节点，其中子节点下面又可以嵌套更多的子节点，这样便形成了一定的层级关系，这种结构看起来像是一颗翻转过来的树，所以被称为树结构。",-1),h=s("br",null,null,-1),y=s("p",null,"如图所示，在树结构里，A 节点包含了 B 节点和 C 节点，B 节点下面又包含了三个其他的子节点，数据一层一层往下扩展。",-1),f=a(`<p>在树结构里还有一种特例，就是二叉树。二叉树是指所有的节点最多只能有两个子节点，这种树结构我们便称之为二叉树。二叉树是面试中经常会被问到的考题，但是对普通的树形结构的处理反而是我们测试中经常会用到的。所以二叉树的处理和普通树结构的处理，你都需要掌握。</p><br><p>那么普通的树结构在什么地方会用到呢？比如我们打开一个网站，这个网站有很多页面，这些页面又都是层级逻辑关系，也就是进入一层界面处理一层界面的操作逻辑，然后又进入另外一个界面，所有的界面的层级便构成了一个复杂的树结构。产品经理在设计 App 或网站的时候，也是一个层级结构。除此之外，还有很多我们测试中用到的结构其实也都是树结构。</p><h2 id="二叉树" tabindex="-1">二叉树 <a class="header-anchor" href="#二叉树" aria-label="Permalink to &quot;二叉树&quot;">​</a></h2><p>讲完了树结构，我们来了解到底什么是二叉树。</p><br><p>树结构其实是由一个核心的数据和一堆指向节点的链接组成。二叉树则是树结构的一种特例，二叉树每个节点下最多只能有两个子节点，所以我们可以使用 left、right 进行表示。如果有更多的子树，你还可以使用 self.children，把所有子节点放到 children 中进行描述。</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki github-dark vp-code-dark"><code><span class="line"><span style="color:#E1E4E8;">class BTree：</span></span>
...
<span class="line"><span style="color:#24292E;">    cls.travel(subtree.left)</span></span>
<span class="line"><span style="color:#24292E;">    print(subtree.data)</span></span>
<span class="line"><span style="color:#24292E;">    cls.travle(subtree.right)</span></span></code></pre></div>`,2),u=a(`<p>如果现在我们把这个节点放到中间呢？再次执行你会注意到这个时候变成了 3、1、4、0、5、2、7、6。这是因为我们此时是按照中序在遍历。</p><br><p>打印的过程中先去寻找左节点，第一个左节点是 3，3 打印完是 1，然后再寻找右节点，接着是 4，最后的结果便是 3、1、4。4 打印完后是 0，以此类推，你可以课后自己推导结果，这里就不再一一介绍了。</p><h2 id="树形结构考题" tabindex="-1">树形结构考题 <a class="header-anchor" href="#树形结构考题" aria-label="Permalink to &quot;树形结构考题&quot;">​</a></h2><p>一个简单的树的遍历，差不多就是这样一个写法，接下来我们剖析一个经典的树结构面试考题。</p><br><p>怎样获取一个树结构的最大深度？其实我们在一些测试工作中有时候也经常会用到这个算法。比如计算某个控件在当前界面中它属于多大的一个深度。接下来我们看下具体怎么解答这个问题。</p><br><p>首先，我们写一个叫作 max depth 的函数，然后我们用递归算法来计算深度。这个深度是这样子的，首先如果它即有左节点，又有右节点，那么每进去一层深度其实是要加 1 的。最后完成之后，我们计算加 1 的值最后能加到多少？你可以看到它的算法本质和遍历是一样的。我们看下它的写法是什么？</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki github-dark vp-code-dark"><code><span class="line"><span style="color:#E1E4E8;">def max_depth(self,subtree,depth):</span></span>
<span class="line"><span style="color:#E1E4E8;">    if subtree is None:</span></span>
...
<span class="line"><span style="color:#24292E;">    self.max_depth(self,0)</span></span></code></pre></div><p>这样使得调用变得更简单。</p><br><p>本课时的内容就讲到这里了，其实核心就是一个树形结构，它的遍历跟链表很相似，子节点跟父节点是类似的算法处理，所以我们可以使用递归算法计算树形的深度。</p>`,23);function _(b,v,T,m,B,k){const e=p("Image");return t(),o("div",null,[i,E,d,h,y,n(e,{alt:"",src:"https://s0.lgstatic.com/i/image3/M01/61/D3/Cgq2xl4f2gWAMlflAAF5FGDHnyo146.png"}),f,n(e,{alt:"",src:"https://s0.lgstatic.com/i/image3/M01/61/D3/Cgq2xl4f2gaAMydMAAEp3l-gCRU140.png"}),g,n(e,{alt:"",src:"https://s0.lgstatic.com/i/image3/M01/61/D2/CgpOIF4f2gaARjLlAAGMP20VB90449.png"}),u])}const q=l(c,[["render",_]]);export{x as __pageData,q as default};
