...
<span class="line"><span style="color:#24292E;">	    baseUrl: http://localhost:9411</span></span></code></pre></div><p>至此，Zipkin 环境已经搭建完毕，我们可以通过访问 <a href="http://localhost:9411" target="_blank" rel="noreferrer">http://localhost:9411</a> 来获取 Zipkin 所提供的所有可视化结果，接下来将演示如何使用 Zipkin 跟踪服务调用链路。</p><h3 id="使用-zipkin-可视化服务调用链路" tabindex="-1">使用 Zipkin 可视化服务调用链路 <a class="header-anchor" href="#使用-zipkin-可视化服务调用链路" aria-label="Permalink to &quot;使用 Zipkin 可视化服务调用链路&quot;">​</a></h3><p>在本课时中，Zipkin 可视化服务调用链路的构建包含三大维度，如下图所示：</p>`,13),k=s("p",null,"构建 Zipkin 可视化服务调用链路的三大维度",-1),v=s("p",null,"接下来，我们将分别这三个维度介绍 Zipkin 的强大功能。",-1),m=s("h4",{id:"可视化服务依赖关系",tabindex:"-1"},[n("可视化服务依赖关系 "),s("a",{class:"header-anchor",href:"#可视化服务依赖关系","aria-label":'Permalink to "可视化服务依赖关系"'},"​")],-1),S=s("p",null,"依赖在某种程度上不可避免，但是过多地依赖势必会增加系统复杂性和降低代码维护性，从而成为团队开发的一种阻碍。在微服务系统中一般存在多个服务，服务需要管理相互之间的依赖关系。当系统规模越来越大后，各个业务服务之间的直接依赖和间接依赖关系就会变得十分复杂。我们需要通过一个简洁明了的可视化工具来查看当前服务链路中的依赖关系，Zipkin 就提供了这方面的支持。",-1),C=s("p",null,[n("在 SpringHealth 案例系统中，当我们通过访问 intervention-service 中的 HTTP 端点"),s("a",{href:"http://localhost:5555/springhealth/intervention/interventions/springhealth_admin/device1",target:"_blank",rel:"noreferrer"},"http://localhost:5555/springhealth/intervention/interventions/springhealth_admin/device1"),n("时，下图展示了通过 Zipkin 获取的服务调用依赖关系：")],-1),Z=e(`<p>Zipkin 中的 SpringHealth 案务依赖关系示意图</p><p>可以看到在这个服务调用链路中，我们首先通过 zuulservice 访问 interventionservice，然后 interventionservice 又通过 zuulservice 分别访问 userservice 和 deviceservice，从而形成一次完整的业务调用。</p><p>那么这张图中的依赖关系是否正确呢？让我们回顾 intervention-service 中生成健康干预信息的代码结构：</p><div class="language-xml vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">xml</span><pre class="shiki github-dark vp-code-dark"><code><span class="line"><span style="color:#E1E4E8;">public Intervention generateIntervention(String userName, String deviceCode) {</span></span>
...
<span class="line"><span style="color:#24292E;">}</span></span></code></pre></div><hr><p>可以看到在 intervention-service 的 generateIntervention 方法中会通过 UserServiceClient 和 DeviceServiceClient 分别访问 user-service 和 device-service。而在这些远程访问中，都将通过 zuul-service 进行路由转发，所以这与上图中的服务依赖关系完全一致。</p><h4 id="可视化服务调用时序" tabindex="-1">可视化服务调用时序 <a class="header-anchor" href="#可视化服务调用时序" aria-label="Permalink to &quot;可视化服务调用时序&quot;">​</a></h4><p>可视化服务调用时序是 Zipkin 最重要的功能，对于服务监控而言，服务调用链数据收集、分析和管理的目的是发现服务调用过程的问题并采取相应的优化措施。下图展示了 Zipkin 可视化服务调用时序的主界面：</p>`,8),b=s("p",null,"Zipkin 可视化服务调用时序的主界面",-1),A=s("p",null,"上图该主界面主体是一个面向查询的操作界面，其中我们需要关注服务名称和端点，因为服务调用链路中的所有服务都会出现在服务名称列表中，同时，针对每个服务，我们也可以选择自身感兴趣的端点信息。同时，我们也发现了多个用于灵活查询的过滤器，包括 TraceId、SpanName、时间访问、调用时长以及标签功能。",-1),f=s("p",null,[n("当然，我们最应该关注的是查询结果。针对某个服务，Zipkin 的查询结果展示了包含该服务的所有调用链路。现在，让我们关注于 user-service 中根据用户名获取用户信息的"),s("a",{href:"http://localhost:5555/springhealth/user/users/username/",target:"_blank",rel:"noreferrer"},"http://localhost:5555/springhealth/user/users/username/"),n("{userName} 端点，Zipkin 上的执行效果图如下所示：")],-1),I=s("p",null,"Zipkin 服务调用链路明细图界面",-1),T=s("p",null,"当发起这个 HTTP 请求时，该请求会先到达 Zuul 网关，然后再通过路由转发到 userservice。通过观察上图服务之间的调用时序，我们在前面介绍的服务依赖关系的基础上给出了更为明确的服务调用关系。",-1),D=s("p",null,"上图中最重要的就是各个 Span 信息。一个服务调用链路被分解成若干个 Span，每个 Span 代表完整调用链路中的一个可以衡量的部分。我们通过可视化的界面，可以看到整个访问链路的整体时长以及各个 Span 所花费的时间。每个 Span 的时延都已经被量化，并通过背景颜色的深浅来表示时延的大小。注意到这里 userservice 出现了两个 Span，原因在于 userservice 在该请求中还访问了 OAuth2 的授权服务器。",-1),N=s("h4",{id:"可视化服务调用数据",tabindex:"-1"},[n("可视化服务调用数据 "),s("a",{class:"header-anchor",href:"#可视化服务调用数据","aria-label":'Permalink to "可视化服务调用数据"'},"​")],-1),x=s("p",null,'在上图中，我们点击任何一个感兴趣的 Span 就可以获取该 Span 对应的各项服务调用数据明细。例如，我们点击"get /users/username/{username}"这个 Span，Zipkin 会跳转到一个新的页面并显示如下图所示的数据：',-1),P=s("p",null,"Zipkin 中 Span 对应的名称、TraceId 和 SpanId",-1),z=s("p",null,'这里看到了本次调用中用于监控的最重要的元数据 TraceId 和 SpanId，以及代表各个关键事件的 Annotation 可视化长条。点击长条下的"SHOW ALL ANNOTATIONS"按钮，可以得到如下所示的事件明细信息：',-1),U=s("p",null,"Zipkin 中 Span 对应的四个关键事件数据界面",-1),w=s("p",null,"上图展示了针对该 Span 的 cs、sr、ss 和 cr 这四个关键事件数据。对于这个 Span 而言，zuulservice 相当于是 userservice 的客户端，所以 zuulservice 触发了 cs 事件，然后通过 (17.160 -- 2.102)ms 到达了 userservice，以此类推。从这些关键事件数据中可以得出一个结论，即该请求的整个服务响应时间主要取决于 userservice 自身的处理时间。",-1),M=s("p",null,"当然，我们针对这个 Span，也可以获取如所示的标签明细数据：",-1),V=e(`<p>Zipkin 中 Span 对应的标签数据</p><p>可以看到这些数据包括 HTTP 请求相关的方法、路径等各项基础数据，也包括使用 Spring 构建 RESTful 风格调用时的 Controller 类以及端点信息。</p><p>最后，作为数据管理和展示的统一平台，Zipkin 还实现了更为低层的数据表现形式，也就是通过 JSON 数据提供对调用过程的详细描述。如下所示的就是一份示例 JSON 数据，描述的也是关键事件所应该具备的各项信息：</p><div class="language-xml vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">xml</span><pre class="shiki github-dark vp-code-dark"><code><span class="line"><span style="color:#E1E4E8;">[</span></span>
<span class="line"><span style="color:#E1E4E8;">     {</span></span>
...
<span class="line"><span style="color:#24292E;"> ]</span></span></code></pre></div><p>在介绍 Zipkin 的基本结构时，我们提到 Zikpin 还提供了专门的 RESTful API 获取一次服务调用链路中所有 Span 对应的 JSON 数据，例如本例中的 SpanName 为&quot;get /users/username/{username}&quot;，所以对应的 API 即为 localhost:9411/zipkin/?serviceName=userservice&amp;spanName=get+%2Fusers%2Fusername%2F{username} ，我们可以根据需要获取所需的各项信息。你可以自己多进行尝试。</p><h3 id="小结与预告" tabindex="-1">小结与预告 <a class="header-anchor" href="#小结与预告" aria-label="Permalink to &quot;小结与预告&quot;">​</a></h3><p>承接上一课时内容，今天的内容重点关注如何实现监控数据的可视化管理。我们引入了 Zipkin 这款优秀的开源框架，并完成与 Spring Cloud Sleuth 的无缝集成。基于 Zipkin，我们可以完成可视化服务依赖关系、可视化服务调用时序和可视化服务调用数据等三大维度的可视化监控功能，本课时对如何实现这些功能进行了详细的展开。</p><p>这里给你留一道思考题：Zipkin 中所提供的可视化功能主要包括哪几个维度？</p><p>到现在为止，尽管我们已经可以利用 Spring Cloud Sleuth 实现了内置的监控功能，但有时候我们可能需要对监控工作做一些定制化的修改和扩展，这时候就需要创建各种自定义的 Span。下一课时我们将来讨论这一话题。</p>`,9);function j(F,O,R,L,B,H){const a=t("Image");return o(),i("div",null,[r,u,E,d,q,p(a,{alt:"Drawing 0.png",src:"https://s0.lgstatic.com/i/image2/M01/04/47/Cip5yF_sSf6AO7sOAAAsmdX5mFU432.png"}),n(),y,h,g,p(a,{alt:"Drawing 1.png",src:"https://s0.lgstatic.com/i/image2/M01/04/49/CgpVE1_sSgeAKwhCAACtM9bH7wM931.png"}),n(),_,p(a,{alt:"Drawing 2.png",src:"https://s0.lgstatic.com/i/image2/M01/04/47/Cip5yF_sShWAETlQAAAyuao3DMc019.png"}),n(),k,v,m,S,C,p(a,{alt:"Drawing 3.png",src:"https://s0.lgstatic.com/i/image2/M01/04/49/CgpVE1_sSh2AVJesAAA5oopzibY911.png"}),n(),Z,p(a,{alt:"Drawing 4.png",src:"https://s0.lgstatic.com/i/image2/M01/04/48/Cip5yF_sSiuAa_zWAABkqhF8VFQ138.png"}),n(),b,A,f,p(a,{alt:"Drawing 5.png",src:"https://s0.lgstatic.com/i/image2/M01/04/48/Cip5yF_sSjOAe2oQAABCtCPP68k747.png"}),n(),I,T,D,N,x,p(a,{alt:"图片6.png",src:"https://s0.lgstatic.com/i/image/M00/8C/83/CgqCHl_tnS6AQDXdAAGtE02WUpo828.png"}),n(),P,z,p(a,{alt:"Drawing 7.png",src:"https://s0.lgstatic.com/i/image/M00/8C/65/Ciqc1F_sSkWAbGxeAABQkYLlisc594.png"}),n(),U,w,M,p(a,{alt:"Drawing 8.png",src:"https://s0.lgstatic.com/i/image/M00/8C/65/Ciqc1F_sSkyAZfeJAAAwoDw1coE218.png"}),n(),V])}const J=l(c,[["render",j]]);export{G as __pageData,J as default};
