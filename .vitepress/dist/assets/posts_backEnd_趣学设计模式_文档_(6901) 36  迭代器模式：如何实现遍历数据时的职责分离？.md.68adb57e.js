import{_ as a,j as p,o as l,g as e,k as t,h as o,Q as s}from"./chunks/framework.4e7d56ce.js";const v=JSON.parse('{"title":"模式原理分析 ","description":"","frontmatter":{},"headers":[],"relativePath":"posts/backEnd/趣学设计模式_文档/(6901) 36  迭代器模式：如何实现遍历数据时的职责分离？.md","filePath":"posts/backEnd/趣学设计模式_文档/(6901) 36  迭代器模式：如何实现遍历数据时的职责分离？.md","lastUpdated":1696417798000}'),c={name:"posts/backEnd/趣学设计模式_文档/(6901) 36  迭代器模式：如何实现遍历数据时的职责分离？.md"},i=s('<p>迭代器模式是我们学习一个设计时很少用到的、但编码实现时却经常使用到的行为型设计模式。<strong>在绝大多数编程语言中，迭代器已经成为一个基础的类库，直接用来遍历集合对象</strong> 。在平时开发中，我们更多的是<strong>直接使用</strong>它，很少会从零去实现一个迭代器。</p><p>不过，我们课程的宗旨就是要弄懂原理，更好地帮助你去灵活使用这些设计模式。所以，今天我会<strong>重点讲解迭代器的原理和实现</strong>。</p><p>话不多说，让我们开始今天的学习吧。</p><h3 id="模式原理分析" tabindex="-1">模式原理分析 <a class="header-anchor" href="#模式原理分析" aria-label="Permalink to &quot;模式原理分析&quot;">​</a></h3><p>迭代器模式又叫游标（Cursor）模式，它的原始定义是：迭代器提供一种对容器对象中的各个元素进行访问的方法，而又不需要暴露该对象的内部细节。</p><p>可以看到，该定义很明确地指出，迭代器模式就是为了提供一种通用的访问对象的方式。我们先来看下它的 UML 图：</p>',6),r=s(`<p>迭代器模式的 UML 图</p><p>从该 UML 图中，我们能看出迭代器模式的四个关键角色。</p><ul><li><p><strong>抽象集合类（Aggregate）</strong>：创建和抽象迭代器类相关联的方法，同时可以添加其他集合类需要的方法。</p></li><li><p><strong>具体集合类（ConcreteAggregate）</strong>：实现抽象集合类声明的所有方法，在具体使用集合类时会创建对应具体的迭代器类。</p></li><li><p><strong>抽象迭代器类（Iterator）</strong>：定义统一的迭代器方法 hasNext() 和 next()，用于判断当前集合中是否还有对象以及按顺序读取集合中的当前对象。</p></li><li><p><strong>具体迭代器类（ConcreteIterator）</strong>：实现了抽象迭代器类声明的方法，处理具体集合中对对象位置的偏移以及具体对象数据的传输。</p></li></ul><p>该 UML 图对应的代码实现如下：</p><div class="language-html vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">html</span><pre class="shiki github-dark vp-code-dark"><code><span class="line"><span style="color:#E1E4E8;">public interface Iterator{</span></span>
