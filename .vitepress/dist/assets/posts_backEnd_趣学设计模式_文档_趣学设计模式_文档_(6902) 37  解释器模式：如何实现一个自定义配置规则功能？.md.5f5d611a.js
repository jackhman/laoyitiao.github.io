import{_ as s,o as n,g as a,Q as p}from"./chunks/framework.f949202b.js";const y=JSON.parse('{"title":"模式原理分析 ","description":"","frontmatter":{},"headers":[],"relativePath":"posts/backEnd/趣学设计模式_文档/趣学设计模式_文档/(6902) 37  解释器模式：如何实现一个自定义配置规则功能？.md","filePath":"posts/backEnd/趣学设计模式_文档/趣学设计模式_文档/(6902) 37  解释器模式：如何实现一个自定义配置规则功能？.md","lastUpdated":null}'),l={name:"posts/backEnd/趣学设计模式_文档/趣学设计模式_文档/(6902) 37  解释器模式：如何实现一个自定义配置规则功能？.md"},e=p(`<p>解释器模式使用频率不算高，通常用来描述如何构建一个简单&quot;语言&quot;的语法解释器。它只在一些非常特定的领域被用到，比如编译器、规则引擎、正则表达式、SQL 解析等。不过，了解它的实现原理同样很重要，能帮助你思考<strong>如何通过更简洁的规则来表示复杂的逻辑</strong>。</p><p>话不多说，让我们开始今天的学习吧。</p><h3 id="模式原理分析" tabindex="-1">模式原理分析 <a class="header-anchor" href="#模式原理分析" aria-label="Permalink to &quot;模式原理分析&quot;">​</a></h3><p>解释器模式的原始定义是：用于定义语言的语法规则表示，并提供解释器来处理句子中的语法。</p><p>语法也称文法，在语言学中指任意自然语言中句子、短语以及词等语法单位的语法结构与语法意义的规律。比如，在编程语言中，if-else 用作条件判断的语法，for 用于循环语句的语法标识。再比如，&quot;我爱中国&quot;是一个中文句子，我们可以用名词、动词、形容词等语法规则来直观地描述句子。</p><p>我们还是直接先来看看解释器模式的 UML 图：</p><p><img src="https://s0.lgstatic.com/i/image6/M00/4E/05/CioPOWDxTw6Ab2ULAAB6RYJCFSY881.png" alt="【配图】37 解释器模式.png"><br> 解释器模式的 UML 图</p><p>从该 UML 图中，我们能看出解释器模式包含的关键角色有四个。</p><ul><li><p><strong>抽象表达式（AbstractExpression）</strong>：定义一个解释器有哪些操作，可以是抽象类或接口，同时说明只要继承或实现的子节点都需要实现这些操作方法。</p></li><li><p><strong>终结符表达式（TerminalExpression）</strong>：用于解释所有终结符表达式。</p></li><li><p><strong>非终结符表达式（NonterminalExpression）</strong>：用于解释所有非终结符表达式。</p></li><li><p><strong>上下文（Context）</strong>：包含解释器全局的信息。</p></li></ul><p>解释器模式 UML 对应的代码实现如下：</p><div class="language-html vp-adaptive-theme line-numbers-mode"><button title="Copy Code" class="copy"></button><span class="lang">html</span><pre class="shiki github-dark vp-code-dark"><code><span class="line"><span style="color:#E1E4E8;">//抽象表达式类</span></span>
