...
<span class="line"><span style="color:#24292E;">        return maxEndingHere;</span></span>
<span class="line"><span style="color:#24292E;">    }</span></span>
<span class="line"><span style="color:#24292E;">}</span></span></code></pre></div><h2 id="时间复杂度-1" tabindex="-1">时间复杂度 <a class="header-anchor" href="#时间复杂度-1" aria-label="Permalink to &quot;时间复杂度&quot;">​</a></h2><p>分析递归+记忆化的时间复杂度，如下。</p>`,41),H=p(`<ol><li><p>函数 f 按序传递n，n−1，n−2 ... 最后是 1，把结果缓存并返回；</p></li><li><p>递归返回到输入 n；</p></li><li><p>缓存里已经保存了 n−1 个结果；</p></li><li><p>for 循环调用递归函数 n−1 次，从 cache 里直接返回结果。</p></li></ol><br><p>上述过程的时间复杂度是 O(1)。即将问题的规模大小从 n 逐渐减小到 1 的时候，通过将各个结果保存起来，可以将 T(1)，T(2)，....T(n−1) 的复杂度降低到线性的复杂度。</p><p>现在，回到 T(n)，在 for 循环里，尝试着从 T(1)，T(2)....T(n−1) 里取出最大值，因此 O(T(n))=O(T(1) + T(2) + ... + T(n−1))=O(1 + 2 + .... + n−1)=O(n×(n−1)/2)=O(n^2^)。</p><p>最后加上构建缓存 cache 的时间，整体的时间复杂度就是 O(f(n))=O(n) + O(n^2)=O(n^2^)。</p><p>通过记忆化的操作，我们把时间复杂度从 O(2^n^) 降低到了 O(n^2^)。</p><p>这种将问题规模不断减少的做法，被称为自顶向下的方法。但是，由于有了递归的存在，程序运行时对堆栈的消耗以及处理是很慢的，在实际工作中并不推荐。更好的办法是自底向上。</p><h6 id="自底向上-bottom-up" tabindex="-1">自底向上（Bottom-Up） <a class="header-anchor" href="#自底向上-bottom-up" aria-label="Permalink to &quot;自底向上（Bottom-Up）&quot;">​</a></h6><p>自底向上指，通过状态转移方程，从最小的问题规模入手，不断地增加问题规模，直到所要求的问题规模为止。依然使用记忆化避免重复的计算，不需要递归。</p><p>建议：在面试的时候，如果能最终给出一个自底向上的方案和代码，则比较完美。</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki github-dark vp-code-dark"><code><span class="line"><span style="color:#E1E4E8;">class LISDP {</span></span>
